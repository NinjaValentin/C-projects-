...
void AddFileToArch(FILE *Arch, char **argv, int j) {
	...
	//сдвиг позиции в архивном файле для записи размера файла
	SizePos = ftell(Arch);
	//измеряем размер
	...
	//Если размер больше 1024 то сжимаем
	if (inf.Fsize > 512) {
		inf.ComFlag = 1;
		...
		//перемещене позиции указателя внутри архива в место для записи размера файла
...
//Удаление файла из архива
 void RemoveFile(char **argv) {
	 ...
	 j = 3; //потому что именя файлов начинаются с третьего аргумента	
 ...
 void extract(FILE *Arch, char **argv) {
	 ...
	 fseek(Arch, 4, SEEK_SET);                //ставим позицию в начало
	 ...
	 while (argv[j]) {
		 ...
		 j++;
		 fseek(Arch, 4, SEEK_SET);//позицию в начала файла
		 size = 4; //обнуление счктчика по размеру файлов
	 }
 }
...
//Вывод информации о файлах в архиве
